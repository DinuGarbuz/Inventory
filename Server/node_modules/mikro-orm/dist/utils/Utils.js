...
class Utils {
    ...
    static shouldIgnoreProperty(entity, prop) {
        if (!(prop.name in entity) || prop.persist === false) {
            return true;
        }
        const collection = entity[prop.name] instanceof entity_1.ArrayCollection;
        const noPkRef = Utils.isEntity(entity[prop.name]) && !entity_1.wrap(entity[prop.name]).__primaryKeys.every(pk => pk);
    ...
    static extractPK(data, meta) {
        ...
        if (Utils.isEntity(data, true) && !meta) {
            meta = entity_1.wrap(data).__meta;
        }
    ...
    static getCompositeKeyHash(entity, meta) {
        const pks = meta.primaryKeys.map(pk => {
            if (Utils.isEntity(entity[pk], true)) {
                return entity_1.wrap(entity[pk]).__serializedPrimaryKey;
            }
            return entity[pk];
        });
    ...
    static getPrimaryKeyValue(entity, primaryKeys) {
        ...
        if (Utils.isEntity(entity[primaryKeys[0]])) {
            return entity_1.wrap(entity[primaryKeys[0]]).__primaryKey;
        }
    ...
    static getPrimaryKeyValues(entity, primaryKeys, allowScalar = false) {
        ...
        return primaryKeys.map(pk => {
            if (Utils.isEntity(entity[pk])) {
                return entity_1.wrap(entity[pk]).__primaryKey;
            }
            return entity[pk];
        });
    }
